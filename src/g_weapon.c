...
void bb_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf);
void bb_blast(edict_t *ent);
...
//        fire_lead (self, start, aimdir, damage, kick, TE_GUNSHOT, hspread, vspread, mod);
		bb_throw (self, start, aimdir, damage, 1500, hspread, vspread, 0);
}
...
{
	setFFState(self); 
	bb_throw (self, start, aimdir, damage, 2000, hspread, vspread, 0);
}
...
// ---
        if (other->takedamage)
        {
                T_Damage (other, ent, ent->owner, ent->velocity, ent->s.origin, plane->normal, ent->dmg*30, 0, 0, MOD_KNIFE_THROWN);
        }
        else if (bbspark->value!=0)
        {
                gi.WriteByte (svc_temp_entity);
...
void bb_throw (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int hspread, int vspread, int sil)
{
        edict_t *bb;
        vec3_t  forward, right, up;
        trace_t tr;
        ...
        VectorNormalize (dir);
        VectorCopy (start, bb->s.origin);
        VectorCopy (start, bb->s.old_origin);
        vectoangles (dir, bb->s.angles);
        VectorScale (dir, speed, bb->velocity);
		bb->gravity = 0.2;
...
//		{
			...
			{
			r = crandom()*hspread/10.0f;
			u = crandom()*vspread/10.0f;
			f = 0.0f;
			}
//		}
//		else
...
//		}
		...
		AngleVectors (bb->s.angles, forward, right, up);     
        VectorMA (bb->velocity, r, up, bb->velocity);
        VectorMA (bb->velocity, u, right, bb->velocity);
        VectorMA (bb->velocity, f, forward, bb->velocity);
        
//		if (blast!=0)
//			VectorSet (bb->s.angles, crandom()*360, crandom()*360, crandom()*360);     

        bb->movetype = MOVETYPE_TOSS;
        bb->clipmask = MASK_SHOT;
        bb->solid = SOLID_BBOX;
        bb->s.effects = 0; //EF_ROTATE?
        VectorClear (bb->mins);
        VectorClear (bb->maxs);
		if (sil==0)
	        bb->s.modelindex = gi.modelindex ("models/objects/bb/tris.md2");
		else
	        bb->s.modelindex = gi.modelindex ("models/objects/bbt/tris.md2");

        bb->owner = self;
        bb->touch = bb_touch;
        bb->nextthink = level.time + 8000/speed;
        bb->think = G_FreeEdict;
        bb->dmg = damage;
        VectorClear (bb->mins);
        VectorClear (bb->maxs);
        bb->s.sound = gi.soundindex ("misc/flyloop.wav");
        bb->classname = "thrown_knife";
...
//              check_dodge (self, rocket->s.origin, dir, speed);
        ...
        PRETRACE();
        tr = gi.trace (self->s.origin, NULL, NULL, bb->s.origin, bb, MASK_SHOT);
        POSTTRACE();
        if (tr.fraction < 1.0)
        {
                VectorMA (bb->s.origin, -10, dir, bb->s.origin);
                bb->touch (bb, tr.ent, NULL, NULL);
        }
...
//    vectoangles (ent->velocity, forward);
	...
	for (i=0; i<40; i++)
		bb_throw( ent, ent->s.origin, forward, 2000, 2000, 0, 0, 1);
