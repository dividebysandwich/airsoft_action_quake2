...
void fire_bb (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int hspread, int vspread, int mod, int sil);
                ...
                case MK23_NUM:
                                        {
                                                ...
                                                if(ent->client->ps.gunframe == 3) // 3
                                                {
                                                        ...
                                                        {
                                                                gi.sound(ent, CHAN_WEAPON, gi.soundindex("weapons/mk23slde.wav"), 1, ATTN_NORM, 0);
                                                        }
                                                        else
                                                        {
...
void Machinegun_Fire (edict_t *ent)
{
        ...
        P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
        fire_bb (ent, start, forward, damage, kick, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, MOD_MACHINEGUN, 0);
...
void Chaingun_Fire (edict_t *ent)
{
        ...
        for (i=0 ; i<shots ; i++)
        {
                ...
                fire_bb (ent, start, forward, damage, kick, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, MOD_CHAINGUN, 0);
        }
...
//      safe_cprintf(ent, PRINT_HIGH, "Spread is %d\n", spread);
        ...
        if ((ent->client->mk23_rds == 1))
        {
		        ...
		        if ( ent->client->pers.inventory[ITEM_INDEX(FindItem(SIL_NAME))] )
					fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_MK23, 1);
				else
					fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_MK23, 0);
                ent->client->mk23_rds--;
                ent->client->dual_rds--;
        }
        else
...
void MP5_Fire(edict_t *ent)
{
		...
		if ( ent->client->pers.inventory[ITEM_INDEX(FindItem(SIL_NAME))] )
	        fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_MP5, 1);
		else
			fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_MP5, 0);
        ent->client->mp5_rds--;
...
void M4_Fire(edict_t *ent)
{
        ...
        P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);    
                ...
                fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_M4, 0);
                ent->client->m4_rds--;
...
void HC_Fire (edict_t *ent)
{
	...
	setFFState(ent);
        ...
        gi.sound(ent, CHAN_WEAPON, gi.soundindex("weapons/cfire.wav"), 1, ATTN_NORM, 0);
        // send muzzle flash
//        gi.WriteByte (svc_muzzleflash);
//        gi.WriteShort (ent-g_edicts);
//        gi.WriteByte (MZ_SSHOTGUN | is_silenced);
...
void Sniper_Fire(edict_t *ent)
{
        ...
        
        {
                //If no reload, fire normally.
				if ( ent->client->pers.inventory[ITEM_INDEX(FindItem(SIL_NAME))] )
					fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_SNIPER, 1);
				else
					fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_SNIPER, 0);
...
void Dual_Fire(edict_t *ent)
{
        ...
        if ( ent->client->ps.gunframe == 8 )
        {
                ...
                if ( ent->client->dual_rds > 1 )
                {
                        ...
                        fire_bb (ent, start, forward, damage, kick, spread, spread,MOD_DUAL, 0);
...
//	VectorAdd (offset, vec3_origin, offset);
//	forward[2] += .17;
//    fire_bullet (ent, start, forward, damage, kick, spread, spread, MOD);
//	Knife_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);

	if (mod==MOD_MK23)
		bb_throw (self, start, aimdir, 300, 1200, hspread, vspread, sil);
	else if (mod==MOD_MP5)
		bb_throw (self, start, aimdir, 300, 1300, hspread, vspread, sil);
	else if (mod==MOD_M4)
		bb_throw (self, start, aimdir, 300, 1400, hspread, vspread, sil);
	else if (mod==MOD_SNIPER)
		bb_throw (self, start, aimdir, 300, 2000, hspread, vspread, sil);
	else if (mod==MOD_DUAL)
		bb_throw (self, start, aimdir, 300, 1200, hspread, vspread, sil);
	else
		bb_throw (self, start, aimdir, 300, 1300, hspread, vspread, sil);
