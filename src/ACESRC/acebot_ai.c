...
//	tr = gi.trace (self->s.origin, vec3_origin, vec3_origin, self->enemy->s.origin, self, MASK_SOLID|MASK_OPAQUE); // Werewolf: was enabled
	tr = gi.trace (self->s.origin, tv(-8,-8,-8), tv(8,8,8), self->enemy->s.origin, self, MASK_PLAYERSOLID|MASK_SOLID|MASK_OPAQUE);

	//We would hit something
	if (tr.fraction < 1.0)	//Werewolf: was 0.9
		//If we're in teamplay the the accidentally hit player is a teammate, hold fire
		if( (teamplay->value) && (OnSameTeam( self, tr.ent)) && (tr.ent->solid==SOLID_BSP) )
			return false;
